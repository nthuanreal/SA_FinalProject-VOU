...
type GranPointsRequest struct {
	GameId  string `json:"game_id"`
	Points  int64  `json:"points"`
	EventId int64  `json:"event_id"`
}

func (handler *Handler) GrantPoints(ctx *gin.Context) {
	...
	// parse request
	var request GranPointsRequest
	if err := ctx.ShouldBindJSON(&request); err != nil {
		ctx.JSON(http.StatusBadRequest, utils.FailResponse(err.Error()))
		return
	}

	response, err := handler.service.GrantPoints(ctx, &api.GrantPointsRequest{
		UserId:  userId.(int64),
